...
DBT_PROJECT_DIR = '/home/mahmoud/dbt_project'

# Task 1: Run dbt models
dbt_run = BashOperator(
    task_id='dbt_run',
    bash_command=f'cd {DBT_PROJECT_DIR} && source dbt-env/bin/activate && cd snowflake_project/ && dbt run',
    dag=dag,
)
...
dbt_test = BashOperator(
    task_id='dbt_test',
    bash_command=f'cd {DBT_PROJECT_DIR} && source dbt-env/bin/activate && cd snowflake_project/ && dbt test',
    dag=dag,
)
